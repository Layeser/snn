#!/usr/bin/env python3
"""
Entraînement A²OS²A (Spiking Transformer, Guo et al. CVPR 2025) sur CIFAR-10.
...
def build_parser(config: dict[str, Any]) -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Train A2OS2A on CIFAR-10")
...
def main():
    args, config = parse_train_args(
        DEFAULT_CONFIG_PATH,
        A2OS2A_CONFIG_SCHEMA,
        build_parser,
        extra_validators=[validate_a2os2a_config],
    )
    ...
    run_training(
        ...
        validate=validate,
        mismatch_keys=("embed_dim", "depth", "num_heads", "T", "batch_size"),
