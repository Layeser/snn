#!/usr/bin/env bash
# =============================================================
# Orchestrateur chicorée — file d'attente + parallélisme GPU
#
# Déposez vos scripts .sh dans :
#   scrip_grid_5000/chicoree_experiences/*.sh
#
# Usage depuis flille (job OAR déjà Running) :
#   bash scrip_grid_5000/run_chicoree_queue.sh --job-id <JOB_ID>
#
# Usage déjà sur le nœud chicorée (après oarsub -C) :
#   cd ~/internship/snn
#   bash scrip_grid_5000/run_chicoree_queue.sh
#
# Options :
#   --job-id ID     Connexion OAR + lancement (depuis flille)
#   --max-gpus N    Slots parallèles (défaut : auto, max 4)
#   --no-git-pull   Ne pas faire git pull avant de démarrer
...
QUEUE_DIR="${CHICOREE_QUEUE_DIR:-${SCRIPT_DIR}/chicoree_experiences}"
ARCHIVE_OK="${QUEUE_DIR}/archive/done"
ARCHIVE_FAIL="${QUEUE_DIR}/archive/failed"
LOG_ROOT="${CHICOREE_LOG_DIR:-${PROJECT_DIR}/outputs/chicoree_queue}"
START_RUN="${SCRIPT_DIR}/start_run.sh"
SENTINEL='=== EXPERIENCE TERMINEE AVEC SUCCES ==='

JOB_ID=""
MAX_GPUS="${CHICOREE_MAX_GPUS:-}"
DO_GIT_PULL=1
DRY_RUN=0
...
connect_and_run() {
    ...
    if [[ "$state" != "Running" ]]; then
        ...
        echo "Attendez l'heure de début ou vérifiez : oarstat -fj ${JOB_ID}" >&2
        exit 1
    fi
    ...
    remote_cmd+=$(printf ' && bash %q --no-git-pull' "$SCRIPT_DIR/run_chicoree_queue.sh")
    ...
    echo "Connexion au job OAR ${JOB_ID} et lancement de l'orchestrateur..."
...
# --- Exécution sur le nœud chicorée ---
cd "$PROJECT_DIR"
...
echo "[$(date -Iseconds)] Démarrage orchestrateur (${#PENDING[@]} jobs, ${MAX_GPUS} GPU)" >>"$SCHEDULER_LOG"
...
reap_finished() {
    ...
    for pid in "${!PID_TO_GPU[@]}"; do
        ...
        if [[ -n "$log_file" ]] && grep -qF "$SENTINEL" "$log_file" 2>/dev/null; then
            status="OK"
            archived="$(archive_script "$script" OK "$ARCHIVE_OK")"
            OK_COUNT=$((OK_COUNT + 1))
        else
            status="FAIL"
            archived="$(archive_script "$script" FAIL "$ARCHIVE_FAIL")"
