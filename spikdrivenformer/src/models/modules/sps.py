...
class SPS(nn.Module):
    """
    MS_SPS — Spiking Patch Splitting (Spike-Driven Transformer).

    Entrée  : (T, B, C_in, H, W)   ex. CIFAR → (4, B, 3, 32, 32)
    Sortie  : (T, B, D, H', W')    ex. pooling_stat="0011" → (4, B, 256, 8, 8)

    pooling_stat : chaîne de 4 caractères '0'/'1' — pool après chaque bloc conv
    """
    ...
    def forward(self, x):
        T, B, _, H, W = x.shape  # ex. (4, B, 3, 32, 32)
        ratio = 1

        x = self.proj_conv(x.flatten(0, 1))                          # (T*B, D/8, H, W)
        x = self.proj_bn(x).reshape(T, B, -1, H // ratio, W // ratio).contiguous()
        x = self.proj_lif(x)                                         # (T, B, D/8, H, W)
        x = x.flatten(0, 1)
        if self.pooling_stat[0] == "1":
            x = self.maxpool(x)                                      # (T*B, D/8, H/2, W/2)
            ratio *= 2

        x = self.proj_conv1(x)                                       # (T*B, D/4, H/r, W/r)
        ...
        x = self.proj_conv2(x)                                       # (T*B, D/2, H/r, W/r)
        ...
        x = self.proj_conv3(x)                                       # (T*B, D, H/r, W/r)
        x = self.proj_bn3(x)
        if self.pooling_stat[3] == "1":
            x = self.maxpool3(x)
            ratio *= 2

        x_feat = x                                                     # skip connection
        x = self.proj_lif3(x.reshape(T, B, -1, H // ratio, W // ratio).contiguous())
        x = x.flatten(0, 1)
        x = self.rpe_conv(x)                                         # (T*B, D, H/r, W/r)
        x = self.rpe_bn(x)
